...
function drawProjectiles(){
    for(const p of projectiles){
        try{
            ...
            if(p.isPlayer){
                const sprite = SPRITES.projectilePlayer;
                if(sprite) {
                    ctx.drawImage(sprite, -4, -sz, 8, sz * 2.5);
                } else {
                ...
                const sprite = SPRITES.projectileEnemy;
                if(sprite) {
                    ctx.drawImage(sprite, -sz*0.7, -sz*1.2, sz*1.4, sz*2.4);
                } else {
                    ctx.fillStyle=p.color||'#fff';ctx.shadowColor=p.color||'#fff';ctx.shadowBlur=12;
